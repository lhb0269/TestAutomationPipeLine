...
async function runNaverLoginFailureTest() {
    let browser;
    let context;
    let page;
    ...
    try {
        console.log('========== TC-022: 네이버 로그인 실패 시나리오 테스트 시작 ==========');
        ...
        await page.screenshot({ 
            path: 'C:\\AIChallenge(QA)\\Docs\\Screenshot\\tc022_01_initial_login_page.png',
            fullPage: true 
        });
        ...
        const naverButtonSelectors = [
            ...
            '.sns-login a:last-child',                    // SNS 로그인 마지막 링크
        ...
        // 스크린샷 캡처 - 네이버 인증 페이지
        await page.screenshot({ 
            path: 'C:\\AIChallenge(QA)\\Docs\\Screenshot\\tc022_02_naver_auth_page.png',
            fullPage: true 
        });
        console.log('   - 네이버 인증 페이지 스크린샷 저장 완료');
        
        // 스텝 4: 잘못된 네이버 계정 정보 입력
        console.log('5. 잘못된 네이버 계정 정보 입력 중...');
        ...
        // 잘못된 네이버 계정 정보 입력
        console.log('   - 잘못된 네이버 아이디 입력 중: invaliduser');
        await loginIdField.fill('');  // 기존 값 지우기
        await loginIdField.type('invaliduser', { delay: 100 });
        
        console.log('   - 잘못된 네이버 비밀번호 입력 중: wrongpass');
        await passwordField.fill('');  // 기존 값 지우기
        await passwordField.type('wrongpass', { delay: 100 });
        
        console.log('   - 잘못된 네이버 계정 정보 입력 완료');
        
        // 스크린샷 캡처 - 잘못된 계정 정보 입력 완료
        await page.screenshot({ 
            path: 'C:\\AIChallenge(QA)\\Docs\\Screenshot\\tc022_03_invalid_credentials_entered.png',
            fullPage: true 
        });
        ...
        // 스텝 6: 에러 메시지 확인
        console.log('8. 네이버 로그인 실패 에러 메시지 확인 중...');
        ...
        let errorMessage = '';
        let isStillOnNaver = false;
        let isRetryable = false;
        
        // 네이버 페이지에 여전히 머물러 있는지 확인
        if (afterLoginURL.includes('naver.com') || afterLoginURL.includes('nid.naver.com')) {
            isStillOnNaver = true;
            console.log('   ✅ 네이버 인증 페이지에 여전히 머물러 있음 (로그인 실패 예상)');
            
            // 다양한 에러 메시지 선택자로 확인
            const errorSelectors = [
                '.error_txt',                        // 네이버 일반 에러 텍스트
                '.error_message',                    // 네이버 에러 메시지
                ...
                'div:has-text("존재하지"), div:has-text("확인"), div:has-text("일치하지")',  // 계정 관련 에러
                '.txt_error',                        // 에러 텍스트 클래스
                '[data-error]',                      // 데이터 에러 속성
                '.msg_error',                        // 네이버 특정 에러 메시지
                '.error_login'                       // 로그인 에러 네이버 스타일
            ];
            ...
            console.log('   ⚠️ 네이버 페이지에서 다른 페이지로 이동됨');
        ...
        if (isStillOnNaver && !isRetryable) {
            try {
                const retryLoginButton = await page.$(naverLoginButtonSelectors[0]) || 
                                       await page.$(naverLoginButtonSelectors[1]);
        ...
        // 스크린샷 캡처 - 에러 메시지 확인
        await page.screenshot({ 
            path: 'C:\\AIChallenge(QA)\\Docs\\Screenshot\\tc022_04_error_message.png',
            fullPage: true 
        });
        ...
        // 최종 스크린샷 캡처
        await page.screenshot({ 
            path: 'C:\\AIChallenge(QA)\\Docs\\Screenshot\\tc022_05_final_result.png',
            fullPage: true 
        });
        ...
        // 테스트 결과 요약
        console.log('\n========== 테스트 결과 요약 ==========');
        console.log(`테스트 케이스: TC-022 네이버 로그인 실패 시나리오`);
        console.log(`사용된 잘못된 계정: invaliduser / wrongpass`);
        console.log(`네이버 버튼 선택자: ${naverButtonSelector}`);
        console.log(`새 탭 열림 여부: ${isNewTabOpened ? '예' : '아니오'}`);
        console.log(`로그인 시도 전 URL: ${beforeLoginURL}`);
        console.log(`로그인 시도 후 URL: ${afterLoginURL}`);
        console.log(`네이버 페이지 유지 여부: ${isStillOnNaver ? '예' : '아니오'}`);
        ...
        // 최종 판정
        const expectedBehavior = isStillOnNaver && (errorFound || isRetryable) && !afterLoginURL.includes('albamon.com');
        
        if (expectedBehavior) {
            console.log('✅ TC-022 테스트 PASS: 네이버 로그인 실패 시나리오가 예상대로 동작');
            console.log('   - 네이버 페이지에서 로그인 실패 처리');
            console.log('   - 알바몬으로 복귀하지 않음');
            console.log('   - 재시도 가능한 상태 유지');
        } else if (afterLoginURL.includes('albamon.com')) {
            console.log('⚠️ TC-022 테스트 UNEXPECTED: 예상과 다르게 알바몬으로 이동됨');
            console.log('   - 잘못된 계정 정보임에도 로그인이 성공했을 가능성');
        } else {
            console.log('❌ TC-022 테스트 PARTIAL: 일부 예상 동작과 다름');
        ...
        if (page) {
            try {
                await page.screenshot({ 
                    path: 'C:\\AIChallenge(QA)\\Docs\\Screenshot\\tc022_error.png',
                    fullPage: true 
                });
        ...
        console.log('❌ TC-022 테스트 ERROR: 실행 중 오류 발생');
        ...
        console.log('========== TC-022 테스트 완료 ==========\n');
    }
}

// 테스트 실행
if (require.main === module) {
    runNaverLoginFailureTest().catch(console.error);
}

module.exports = { runNaverLoginFailureTest };