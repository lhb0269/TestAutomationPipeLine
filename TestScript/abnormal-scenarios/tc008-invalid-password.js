 ...
 * 1. 알바몬 로그인 페이지 접속
 * 2. 아이디 입력 필드에 '' 입력
 * 3. 비밀번호 입력 필드에 'wrongpassword' 입력
 * 4. 로그인 버튼 클릭
 * 5. 서버 응답 대기
...
async function runInvalidPasswordTest() {
    let browser;
    let context;
    let page;
    
    try {
        console.log('========== TC-008: 잘못된 비밀번호로 로그인 시도 테스트 시작 ==========');
        ...
        await page.screenshot({ 
            path: 'C:\\AIChallenge(QA)\\Docs\\Screenshot\\tc008_01_initial_page.png',
            fullPage: true 
        });
        console.log('   - 초기 페이지 스크린샷 저장 완료');
        
        // 스텝 2: 아이디 입력 필드 확인 및 올바른 아이디 입력
        console.log('3. 아이디 입력 필드에 올바른 아이디 입력 중...');
        ...
        await page.type(userIdSelector, validUserId, { delay: 100 });
        console.log(`   - 올바른 아이디 "${validUserId}" 입력 완료`);
        
        // 스크린샷 캡처 - 올바른 아이디 입력 완료
        await page.screenshot({ 
            path: 'C:\\AIChallenge(QA)\\Docs\\Screenshot\\tc008_02_valid_userid_entered.png',
            fullPage: true 
        });
        console.log('   - 올바른 아이디 입력 스크린샷 저장 완료');
        
        // 스텝 3: 비밀번호 입력 필드 확인 및 잘못된 비밀번호 입력
        console.log('4. 비밀번호 입력 필드 확인 및 잘못된 비밀번호 입력 중...');
        ...
        await page.type(passwordSelector, invalidPassword, { delay: 100 });
        console.log(`   - 잘못된 비밀번호 "${invalidPassword}" 입력 완료`);
        
        // 스크린샷 캡처 - 올바른 아이디와 잘못된 비밀번호 입력 완료
        await page.screenshot({ 
            path: 'C:\\AIChallenge(QA)\\Docs\\Screenshot\\tc008_03_both_fields_entered.png',
            fullPage: true 
        });
        ...
        // 최종 스크린샷 캡처
        await page.screenshot({ 
            path: 'C:\\AIChallenge(QA)\\Docs\\Screenshot\\tc008_04_final_result.png',
            fullPage: true 
        });
        ...
        try {
            ...
            if (finalUserIdValue === validUserId) {
                console.log('   ✅ 올바른 아이디가 그대로 유지됨');
                testDetails.push('아이디 입력값 유지 확인');
            } else {
                console.log('   ⚠️ 아이디 필드 값이 변경됨');
            }
            
            if (finalPasswordValue === invalidPassword) {
                console.log('   ✅ 잘못된 비밀번호가 그대로 유지됨');
                testDetails.push('비밀번호 입력값 유지 확인');
            } else if (finalPasswordValue.length === 0) {
                console.log('   ⚠️ 비밀번호 필드가 초기화됨 (보안 목적일 수 있음)');
                testDetails.push('비밀번호 필드 초기화됨');
            } else {
                console.log('   ⚠️ 비밀번호 필드 값이 변경됨');
            }
        ...
        // 테스트 결과 요약
        console.log('\n========== 테스트 결과 요약 ==========');
        console.log(`테스트 케이스: TC-008 잘못된 비밀번호로 로그인 시도`);
        console.log(`테스트 시나리오: 올바른 아이디(${validUserId}) + 잘못된 비밀번호(${invalidPassword}) + 로그인 시도`);
        ...
        if (testResult === 'PASS') {
            console.log('\n✅ TC-008 테스트 PASS: 잘못된 비밀번호로 로그인 시도 시 적절한 에러 처리 수행');
        } else {
        ...
        if (page) {
            try {
                await page.screenshot({ 
                    path: 'C:\\AIChallenge(QA)\\Docs\\Screenshot\\tc008_error.png',
                    fullPage: true 
                });
        ...
        console.log('❌ TC-008 테스트 ERROR: 실행 중 오류 발생');
        ...
        console.log('========== TC-008 테스트 완료 ==========\n');
    }
}

// 테스트 실행
if (require.main === module) {
    runInvalidPasswordTest().catch(console.error);
}

module.exports = { runInvalidPasswordTest };